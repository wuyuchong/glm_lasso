...
downdating = function(left_mat, k)
{
  ...
  left_mat_k = left_mat[-(k+1),]
  mk = k
  while(mk < p)
  {
    mx = left_mat_k[(mk+1), (mk+1):(mk+2)]
    lmx = sqrt(sum(mx^2))
    left_mat_k[mk + 1, mk + 1] = lmx
    left_mat_k[mk + 1, mk + 2] = 0
    if(mk < p - 1)
    {
      tmp_mat = left_mat_k[(mk + 2):p, (mk + 1):(mk + 2)]
      tmp_mat = tmp_mat %*% gives_tran(mx, lmx)
      left_mat_k[(mk + 2):p, (mk + 1):(mk + 2)] = tmp_mat
    }
    mk = mk + 1
  }
...
updating = function(left_mat, xxk, xkxk)
{
  ...
  left_mat_up = cbind(left_mat, rep(0,10))
  left_mat_down = rbind(lk, lkk) # not sure cbind or rbind
...
lars_init = function(w, xt, cc_t, is_active_t, active_set_t)
{
  ...
  xt_a = xt[is_active_t]
  xtx_a = (xt_a * w) %*% xt_a.T
  return(list(lamb_t, t(chol(xtx_a))))
}
```
...
lars_step = function(xt, w, p, b_t, cc_t, active_set_t, is_active_t, left_mat_t, lamb_t, df_t)
{
  ...
  u_t = d_t %*% xt[active_set_t]
  a_t = xt[!is_active_t] %*% (u_t * w)
  gam = rep(1, p) * lamb_t
  if(df_t > 1)
  {
...
lars_iter = function(y, xt, b_, is_active_, lamb, pmax)
{
  ...
  {
    count1 = count1 + 1
    eta = b_[is_active_] %*% xt[is_active_]
    exp_eta = exp(eta)
    mu = exp_eta / (1 + exp_eta)
    w = mu * (1 - mu)
    z = y - mu
    
    is_active_t = c(TRUE, rep(FALSE, p))
    active_set_t = c(0)
    b_t = rep(0, p)
    b_t[1] = sum(eta * w + z) / sum(w)
    cc_t = (xt %*% ((eta * w) - (b_t[0] * w))) + (xt %*% z)
...
logit_lasso_init = function(x, xt, y, n, p)
{
  ...
  temp = gam[!is_active_]
  temp[ judge] = (lamb - cc[!is_active_]) / (1 - a)
  temp[!judge] = (lamb + cc[!is_active_]) / (1 + a)
  gam[!is_active_] = temp
  rm(temp)
  rm(judge)
